...
namespace NS_Composants {
	...
	String^ CL_TBCOMMANDE::INSERT(void) {
		return "INSERT INTO dbo.Commande " + "(DDate_emission, Date_payement, Mode_payement, Remise, ID_societe_Produit, ID_client) " + "VALUES('" + getDate_emission() + "', '" + getDate_payement() + "', '" + getMode_payement() + "', '" + getRemise() + "', '" + getID_client() + "'); SELECT @@IDENTITY;";
	}
	String^ CL_TBCOMMANDE::UPDATE(void) {
		return "UPDATE dbo.Commande set Date_emission = ' " + getDate_emission()->ToString() + "'" + ", Date_payement = '" + getDate_payement()->ToString() + "'" + ", Mode_payement='" + getMode_payement()->ToString() + "'" + ", Remise = '" + getRemise() + ", ID_client = '" + Convert::ToInt32(getID_client()) + "' WHERE ID_commande = '" + Convert::ToInt32(getID_commande()) + "'";
	}
