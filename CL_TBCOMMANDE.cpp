...
namespace NS_Composants {
	...
	String^ CL_TBCOMMANDE::SELECTbyIDclient(void) {
		return "SELECT Date_emission,Date_payement,Mode_payement,Remise,Reference_commande FROM dbo.Commande WHERE ID_client='"+Convert::ToInt32(getID_client())+"'";
	}
	...
	String^ CL_TBCOMMANDE::INSERT(void) {
		return "INSERT INTO dbo.Commande " + "(Date_emission, Date_payement, Mode_payement, Remise, ID_client,Reference_commande,ID_adresse) " + "VALUES('" + getDate_emission() + "', '" + getDate_payement() + "', '" + getMode_payement() + "', '" + getRemise() + "', '" + getID_client() + "','"+getReference_commande()+"','"+ Convert::ToInt32(getID_adresse())+"'); SELECT @@IDENTITY;";
	}
	String^ CL_TBCOMMANDE::UPDATE(void) {
		return "UPDATE dbo.Commande set Date_emission = ' " + getDate_emission()->ToString() + "'" + ", Date_payement = '" + getDate_payement()->ToString() + "'" + ", Mode_payement='" + getMode_payement()->ToString() + "'" + ", Remise = '" + getRemise() + "', ID_client = '" + Convert::ToInt32(getID_client()) + "', Reference_commande = '"+getReference_commande()+"',ID_adresse = '"+ Convert::ToInt32(getID_adresse())+"' WHERE ID_commande = " + Convert::ToInt32(getID_commande());
	}
